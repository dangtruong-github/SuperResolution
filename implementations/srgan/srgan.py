...
def srgan_train(opt_total):
    ...
    # Optimizers
    optimizer_G = torch.optim.Adam(generator.parameters(), lr=opt_model.lr, betas=(opt_model.b1, opt_model.b2))
    optimizer_D = torch.optim.Adam(discriminator.parameters(), lr=opt_model.lr, betas=(opt_model.b1, opt_model.b2))
    ...
    for epoch in range(opt_model.epoch+1, opt_model.n_epochs+1):
        if epoch >= opt_model.epoch+3 and opt_total.test_mode == True:
            break
        ...
        for i, imgs in enumerate(train_loader):
            if i >= 2 and opt_total.test_mode == True:
                break
            ...
            sys.stdout.write(
                "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [GAN loss: %f] [G loss: %f]\n"
                % (epoch, opt_model.n_epochs, i, len(train_loader), loss_D.item(), loss_GAN.item(), loss_G.item())
            )
            ...
            if batches_done % opt_model.sample_interval == 0:
                ...
                img_grid = make_grid(img_grid, nrow=1, normalize=True)
                save_image(img_grid, "{}/{}.png".format(img_path, batches_done), normalize=False)
        ...
        sys.stdout.write(
            "Total Training Stats: [Epoch %d/%d] [D loss: %f] [GAN loss: %f] [G loss: %f]\n"
            % (epoch, opt_model.n_epochs, train_loss_D, train_loss_GAN, train_loss_G)
        )
        sys.stdout.write(
        ...
        sys.stdout.write(
            "Total Evaluation Stats: [Epoch %d/%d] [D loss: %f] [GAN loss: %f] [G loss: %f]\n"
        ...
        if opt_model.checkpoint_interval != -1 and epoch % opt_model.checkpoint_interval == 0:
