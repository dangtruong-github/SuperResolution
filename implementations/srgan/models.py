...
def GetModel(gen_path=None, dis_path=None):
    # Model preparation
    generator = GeneratorResNet().to(device=device)

    if gen_path is not None:
        checkpoint_g = torch.load(gen_path, map_location=device, weights_only=True)
        generator.load_state_dict(checkpoint_g)

    discriminator = Discriminator(input_shape=(3, 256, 256)).to(device=device)

    if dis_path is not None:
        checkpoint_d = torch.load(dis_path, map_location=device, weights_only=True)
        discriminator.load_state_dict(checkpoint_d)

    discriminator.eval()

    feature_extractor = FeatureExtractor().to(device=device)
